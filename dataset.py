...
def load_batch(batch_path: str):
    input_node_features = np.load(os.path.join(batch_path, "input_node_features.npy"))
    input_edge_features = np.load(os.path.join(batch_path, "input_edge_features.npy"))
    input_graph_features = np.load(os.path.join(batch_path, "input_graph_features.npy"))
    ...
    for i in range(4):
        node_features = np.load(os.path.join(batch_path, f"out_node_features_{i}.npy"))
        node_features_all_layers.append(node_features)

        edge_features = np.load(os.path.join(batch_path, f"out_edge_features_{i}.npy"))
